...
- **Import bug — `from . import api`.** Six files use package-relative imports while sitting at the top of `tools/trader/` (not inside a package): `broker_profile.py`, `macro.py`, `market_structure.py`, `narrative.py`, `psychology.py`, `sid_tracker.py`. Direct import (`python3 -c "import broker_profile"` from `tools/trader/`) fails with `ImportError: attempted relative import with no known parent package`. The `_lib/<name>.py` shims also fail because they re-import the same broken file. Production runtime scripts (`runtime_layer1_context.py`, `runtime_layer2_screening.py`) avoid this by not consuming these modules — but any future consumer (skill/playbook calling them directly) will break. **Suggested fix:** replace `from . import api` with `import api` in each file. Reversible. Out of scope for Phase 4 (audit only) — flag for a dedicated cleanup commit.
